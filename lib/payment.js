...
module.exports = (app, config) => {
  ...
  app.get(config.payPath, async (req, res) => {
    ...
    if (!token) {
      return res.status(400).end('请提供token')
    }
    const getJsConfig = config.getJsConfig(req.url, origin)  // 获取JSConfig
    ...
    // 输出支付页面
    res.render('pay', { title: orderInfo.body, jsConfig, payConfig, redirect: redirectUrl && decodeURIComponent(redirectUrl) })
  })
  /**
   * 支付成功回调middleware
   * @param {*} callback 
